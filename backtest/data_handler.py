...
class DataHandler:
    ...
    def get_data(self,symbols:list[str],start:str="",end:str="",period:str='1m')->pd.DataFrame:
        xtdata.download_history_data2(symbols,period,start_time=start,end_time=end)
        for symbol in symbols:
            xtdata.subscribe_quote(symbol, period=period, start_time=start, end_time=end, callback=None)
        data=xtdata.get_market_data_ex([],symbols,period=period,start_time=start,end_time=end)
        time.sleep(1)
        for symbol in symbols:
